...
contract TicketSystem {
    ...
    struct TicketInfo {
        ...
        uint256 lastPurchaseBlock; // Block number of the last purchase
    }
    ...
    function buyTickets(string calldata bskyHandle, uint256 numberOfTickets) external payable returns (bool) {
        ...
        // Update ticket counts and last purchase block
        info.fundedCount += numberOfTickets;
        info.lastPurchaseBlock = block.number;
    ...
    function getTicketInfo(string calldata bskyHandle) external view returns (
        ...
        return (
            info.fundedCount,
            info.completedCount,
            info.lastPurchaseBlock,
            info.fundedCount - info.completedCount
        );
    }
    
    function getTicketPrice(uint256 numberOfTickets) external pure returns (uint256) {
        return TICKET_PRICE * numberOfTickets;
    }
